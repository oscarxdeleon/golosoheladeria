...
DECLARE
  ...
  v_report_is_newer_instance boolean := false;
...
BEGIN
  ...
  v_report_is_newer_instance := v_instance_id IS NOT NULL
    AND NOT v_same_instance
    AND (
      v_cfg.active_instance_started_at IS NULL
      OR v_started_at > v_cfg.active_instance_started_at
    );

  v_report_is_older_instance := v_recent_active_connection
    AND NOT v_same_instance
    AND NOT v_report_is_newer_instance
    AND (
  ...
  RETURN jsonb_build_object(
    ...
    'pending_command', v_cmd,
    'active_instance_replaced', v_report_is_newer_instance
  );
END;
$function$;
