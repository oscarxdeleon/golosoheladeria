...
ALTER TABLE public.whatsapp_bot_config
  ADD COLUMN IF NOT EXISTS greet_cooldown_hours int NOT NULL DEFAULT 24,
  ADD COLUMN IF NOT EXISTS short_reply_words text[] NOT NULL DEFAULT ARRAY[
    'gracias','graciass','ok','okay','okey','listo','vale','dale','si','sí','no','confirmo','confirmado','perfecto','bueno','buenas','ya','entendido'
  ]::text[];

ALTER TABLE public.whatsapp_bot_greeted
  ADD COLUMN IF NOT EXISTS last_greeted_at timestamptz NOT NULL DEFAULT now(),
  ADD COLUMN IF NOT EXISTS last_msg_idx int;
...
BEGIN
  ...
  -- Detectar mensaje corto de confirmación humana
  ...
  -- Cooldown: revisar último saludo a este número
  ...
  -- Helper para elegir índice rotando sin repetir el anterior
  -- (se usa inline en cada caso)
