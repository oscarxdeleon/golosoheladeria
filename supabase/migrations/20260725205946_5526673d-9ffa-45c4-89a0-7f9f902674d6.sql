...
DECLARE
  ...
  v_recent_connected_strong boolean := false;
  v_is_stale_instance boolean := false;
  v_preserve_connected boolean := false;
BEGIN
  ...
  -- Ventana corta anti-parpadeo: Baileys puede emitir close/connecting/QR durante
  -- reconexiones normales. No debe tumbar el panel si la sede acaba de reportar open.
  v_recent_connected_strong := v_cfg.connected_phone IS NOT NULL
  ...
  v_preserve_connected := v_cfg.connection_status = 'connected'
    AND v_recent_connected_strong
    AND _status IN ('disconnected','connecting','error');

  IF (_status = 'qr' AND v_recent_connected) OR v_is_stale_instance OR v_preserve_connected THEN
    UPDATE public.whatsapp_bot_config
       SET connection_status = CASE WHEN connection_status IN ('qr','connecting','disconnected','error') THEN 'connected' ELSE connection_status END,
           qr_code = NULL,
           last_seen_at = v_now,
    ...
    RETURN jsonb_build_object(
      ...
      'ignored', true,
      'reason', CASE
        WHEN v_is_stale_instance THEN 'stale_instance_status_ignored'
        WHEN _status = 'qr' THEN 'recent_connected_qr_ignored'
        ELSE 'transient_disconnect_ignored'
      END,
