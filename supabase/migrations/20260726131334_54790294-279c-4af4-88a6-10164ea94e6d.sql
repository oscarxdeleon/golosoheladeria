...
-- Índices de rendimiento para las verificaciones de conversación activa
CREATE INDEX IF NOT EXISTS idx_wa_ai_carts_active_phone
  ON public.whatsapp_ai_carts (branch_id, phone, status, updated_at DESC)
  WHERE status = 'active';

CREATE INDEX IF NOT EXISTS idx_wa_ai_messages_recent_phone
  ON public.whatsapp_ai_messages (branch_id, phone, created_at DESC);

-- Refactor de whatsapp_bot_handle_incoming: corrige la causa raíz de la
-- pérdida de contexto. Cuando hay carrito activo o el asistente respondió
-- hace <20 min, nunca se envían plantillas fijas: la IA maneja el turno.
...
BEGIN
  ...
  -- ========================================================================
  -- GATE MAESTRO ANTI-PÉRDIDA DE CONTEXTO
  -- Si el cliente ya tiene un carrito activo, o el asistente respondió en
  -- los últimos 20 minutos, ESTAMOS EN MEDIO DE UNA CONVERSACIÓN.
  -- Ninguna plantilla fija (saludo, menú, cooldown, fuera de horario) debe
  -- interrumpir. Todo va al asistente para preservar el hilo.
  -- ========================================================================
  SELECT EXISTS(
  ...
  IF v_in_active_conversation THEN
    ...
    END IF;
    -- Sin IA autorizada: no responder para no romper contexto humano.
    RETURN jsonb_build_object(
      'reply', null,
  ...
  IF v_last_any IS NOT NULL AND v_last_any > v_now - v_cooldown THEN
    IF v_is_greeting THEN
      -- Aun en cooldown, si hay IA autorizada delegamos para que responda
      -- naturalmente en lugar de silenciar al cliente.
      IF v_ai_authorized THEN
