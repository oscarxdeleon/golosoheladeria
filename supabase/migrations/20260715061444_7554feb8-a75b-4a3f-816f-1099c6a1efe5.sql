...
DECLARE
  ...
  v_result jsonb; v_branches jsonb; v_summary jsonb;
  v_by_hour jsonb; v_by_service jsonb; v_by_payment jsonb;
  v_top jsonb; v_active_cash jsonb; v_recent jsonb; v_movements jsonb;
BEGIN
  ...
  ELSE
    SELECT id INTO v_branch_id FROM branches ORDER BY is_main DESC NULLS LAST, name ASC LIMIT 1;
  END IF;
  ...
  active_sales AS (SELECT * FROM day_sales WHERE COALESCE(status,'completed')<>'cancelled'),
  cancelled_sales AS (SELECT * FROM day_sales WHERE COALESCE(status,'completed')='cancelled'),
  pay_split AS (
    SELECT LOWER(TRIM(COALESCE(sp->>'method','otro'))) AS method,
           COALESCE((sp->>'amount')::numeric,0) AS amount
      FROM active_sales s,
           LATERAL jsonb_array_elements(
             CASE WHEN COALESCE(s.payment_details->>'split','false') = 'true'
                       AND jsonb_typeof(s.payment_details->'splits')='array'
                  THEN s.payment_details->'splits' ELSE '[]'::jsonb END) sp
  ),
  pay_single AS (
    SELECT LOWER(TRIM(COALESCE(payment_method,'otro'))) AS method, COALESCE(total,0) AS amount
      FROM active_sales
     WHERE NOT (COALESCE(payment_details->>'split','false') = 'true'
                AND jsonb_typeof(payment_details->'splits')='array')
        OR payment_details IS NULL
  ),
  pay_all AS (SELECT method, SUM(amount)::numeric AS amount FROM (
      SELECT method, amount FROM pay_split UNION ALL SELECT method, amount FROM pay_single) x GROUP BY method),
  ...
  exp_cash_agg AS (SELECT COALESCE(SUM(amount),0)::numeric AS v FROM expenses
                    WHERE branch_id=v_branch_id AND created_at>=v_start AND created_at<v_end
                      AND (LOWER(COALESCE(payment_method,'efectivo'))='efectivo' OR payment_method IS NULL)),
  pur_agg AS (SELECT COALESCE(SUM(total),0)::numeric AS v FROM purchases
               WHERE branch_id=v_branch_id AND created_at>=v_start AND created_at<v_end),
  pur_cash_agg AS (SELECT COALESCE(SUM(total),0)::numeric AS v FROM purchases
                    WHERE branch_id=v_branch_id AND created_at>=v_start AND created_at<v_end
                      AND (LOWER(COALESCE(payment_method,'efectivo'))='efectivo' OR payment_method IS NULL)),
  ...
  SELECT jsonb_build_object(
    ...
    'expenses', ex.v + pu.v, 'expenses_cash', exc.v + puc.v, 'purchases', pu.v,
    'deposits', dp.v, 'deposits_cash', dpc.v,
    'entries', dp.v, 'exits', ex.v + pu.v, 'refunds', 0, 'tips', tp.v,
    'cancelled_count', ca.n, 'cancelled_value', ca.v,
    'opening_amount', oa.v,
    'expected_cash', oa.v + cs.v + dpc.v - exc.v - puc.v,
    'net_balance', (t.total_sales - ex.v - pu.v + dp.v),
    'tables_occupied', tb.n, 'pending_llevar', pl.n,
    'pending_domicilio', pd.n, 'preparing', pr.n
  ) INTO v_summary
  FROM totals t, cash_sum cs, digital_sum ds, exp_agg ex, exp_cash_agg exc,
       pur_agg pu, pur_cash_agg puc, dep_agg dp, dep_cash_agg dpc, cancel_agg ca,
       tables_occ tb, preparing_agg pr, pending_dom pd, pending_llevar pl, tips_agg tp, opening_amt oa;
  ...
  SELECT COALESCE(jsonb_object_agg(hh, v), '{}'::jsonb) INTO v_by_hour FROM (
    SELECT LPAD(EXTRACT(HOUR FROM (created_at AT TIME ZONE v_tz))::text,2,'0') AS hh,
           SUM(total)::numeric AS v FROM sales
     WHERE branch_id=v_branch_id AND created_at>=v_start AND created_at<v_end AND COALESCE(status,'completed')<>'cancelled'
     GROUP BY 1) q;
  ...
  SELECT COALESCE(jsonb_object_agg(k,v),'{}'::jsonb) INTO v_by_service FROM (
    SELECT COALESCE(NULLIF(order_type,''), NULLIF(source,''),'mesa') AS k, SUM(total)::numeric AS v
      FROM sales WHERE branch_id=v_branch_id AND created_at>=v_start AND created_at<v_end AND COALESCE(status,'completed')<>'cancelled'
     GROUP BY 1) q;

  SELECT COALESCE(jsonb_object_agg(method, amount),'{}'::jsonb) INTO v_by_payment FROM (
    WITH day_sales AS (SELECT * FROM sales WHERE branch_id=v_branch_id AND created_at>=v_start AND created_at<v_end AND COALESCE(status,'completed')<>'cancelled'),
    sp AS (
      SELECT LOWER(TRIM(COALESCE(x->>'method','otro'))) AS method, COALESCE((x->>'amount')::numeric,0) AS amount
        FROM day_sales s, LATERAL jsonb_array_elements(
             CASE WHEN COALESCE(s.payment_details->>'split','false') = 'true'
                       AND jsonb_typeof(s.payment_details->'splits')='array'
                  THEN s.payment_details->'splits' ELSE '[]'::jsonb END) x),
    single AS (
      SELECT LOWER(TRIM(COALESCE(payment_method,'otro'))) AS method, COALESCE(total,0) AS amount FROM day_sales
       WHERE NOT (COALESCE(payment_details->>'split','false') = 'true' AND jsonb_typeof(payment_details->'splits')='array')
          OR payment_details IS NULL)
    SELECT method, SUM(amount)::numeric AS amount FROM (SELECT * FROM sp UNION ALL SELECT * FROM single) u GROUP BY method
  ) q;

  WITH day_sales AS (SELECT id FROM sales WHERE branch_id=v_branch_id AND created_at>=v_start AND created_at<v_end AND COALESCE(status,'completed')<>'cancelled'),
  agg AS (
    SELECT COALESCE(NULLIF(p.name,''), TRIM(SPLIT_PART(COALESCE(si.product_name,''),'+',1))) AS name,
           SUM(COALESCE(si.qty,0))::numeric AS qty, SUM(COALESCE(si.subtotal,0))::numeric AS total
      FROM sale_items si JOIN day_sales ds ON ds.id=si.sale_id
  ...
  SELECT to_jsonb(cs.*) || jsonb_build_object('cash_expected_calc', COALESCE((v_summary->>'expected_cash')::numeric,0))
    INTO v_active_cash FROM cash_sessions cs
   WHERE cs.branch_id=v_branch_id AND cs.status='open' ORDER BY cs.opened_at DESC LIMIT 1;
  ...
  SELECT COALESCE(jsonb_agg(m ORDER BY (m->>'created_at') DESC), '[]'::jsonb)
    INTO v_movements FROM (
      SELECT jsonb_build_object(
        ...
        'method',COALESCE(e.payment_method,'efectivo'),'amount',e.amount,'status','active') AS m
        FROM expenses e WHERE e.branch_id=v_branch_id AND e.created_at>=v_start AND e.created_at<v_end
      UNION ALL
      SELECT jsonb_build_object(
        'id',p.id,'kind','expense','created_at',p.created_at,'user_name',p.user_name,
        'category','compra','description',COALESCE(p.supplier,p.notes,p.invoice_number,'Compra'),
        'method',COALESCE(p.payment_method,'efectivo'),'amount',p.total,'status','active') AS m
        FROM purchases p WHERE p.branch_id=v_branch_id AND p.created_at>=v_start AND p.created_at<v_end
      UNION ALL
      SELECT jsonb_build_object(
        ...
        FROM cash_deposits d WHERE d.branch_id=v_branch_id AND d.created_at>=v_start AND d.created_at<v_end
    ) t;

  v_result := jsonb_build_object(
    'supervisor', jsonb_build_object('username', v_supervisor.username, 'display_name', v_supervisor.display_name),
    'branches', v_branches, 'active_branch_id', v_branch_id, 'generated_at', now(),
    'scope', jsonb_build_object('kind', CASE WHEN _date IS NULL THEN 'today' ELSE 'date' END,
      'date', v_target_date, 'start_at', v_start, 'end_at', v_end, 'timezone', v_tz, 'cash_session_id', NULL),
    'summary', v_summary, 'by_hour', v_by_hour, 'by_service', v_by_service, 'by_payment', v_by_payment,
    'top_products', v_top, 'low_products', '[]'::jsonb,
    'active_cash', v_active_cash, 'recent_closures', v_recent, 'movements', v_movements
  );
  RETURN v_result;
END; $$;
...
BEGIN
  ...
  WITH scoped_sales AS (
    SELECT DISTINCT ON (s.id) s.*
      FROM sales s
     WHERE s.cash_session_id=v_cs.id
        OR (s.branch_id=v_cs.branch_id AND s.created_at>=v_start AND s.created_at<v_end)
  ),
  active_sales AS (SELECT * FROM scoped_sales WHERE COALESCE(status,'completed')<>'cancelled'),
  cancelled_sales AS (SELECT * FROM scoped_sales WHERE COALESCE(status,'completed')='cancelled'),
  split_parts AS (
    SELECT x.id AS sale_id,
           LOWER(TRIM(COALESCE(sp->>'method','otro'))) AS method,
           COALESCE((sp->>'amount')::numeric,0) AS amount,
           ord = 1 AS is_primary
      FROM active_sales x,
           LATERAL jsonb_array_elements(
             CASE WHEN COALESCE(x.payment_details->>'split','false')='true'
                       AND jsonb_typeof(x.payment_details->'splits')='array'
                  THEN x.payment_details->'splits' ELSE '[]'::jsonb END
           ) WITH ORDINALITY AS sp(sp, ord)
  ),
  single_parts AS (
    SELECT id AS sale_id, LOWER(TRIM(COALESCE(payment_method,'otro'))) AS method,
           COALESCE(total,0) AS amount, true AS is_primary
      FROM active_sales
     WHERE NOT (COALESCE(payment_details->>'split','false')='true'
                AND jsonb_typeof(payment_details->'splits')='array')
        OR payment_details IS NULL
  ),
  pay_rows AS (SELECT * FROM split_parts UNION ALL SELECT * FROM single_parts),
  pay_all AS (
    SELECT method,
           SUM(amount)::numeric AS amount,
           COUNT(DISTINCT sale_id) FILTER (WHERE is_primary)::int AS count
      FROM pay_rows GROUP BY method
  ),
  totals AS (SELECT COALESCE(SUM(total),0)::numeric AS total_sales, COUNT(*)::int AS order_count FROM active_sales),
  cancel_agg AS (SELECT COUNT(*)::int AS n, COALESCE(SUM(total),0)::numeric AS v FROM cancelled_sales),
  cash_sum AS (SELECT COALESCE(SUM(amount),0)::numeric AS v FROM pay_all WHERE method='efectivo'),
  ...
  exp_cash AS (SELECT COALESCE(SUM(amount),0)::numeric AS v FROM expenses
                WHERE cash_session_id=v_cs.id AND (LOWER(COALESCE(payment_method,'efectivo'))='efectivo' OR payment_method IS NULL)),
  pur_agg AS (SELECT COALESCE(SUM(total),0)::numeric AS v FROM purchases WHERE cash_session_id=v_cs.id),
  pur_cash AS (SELECT COALESCE(SUM(total),0)::numeric AS v FROM purchases
                WHERE cash_session_id=v_cs.id AND (LOWER(COALESCE(payment_method,'efectivo'))='efectivo' OR payment_method IS NULL)),
  ...
  dep_cash AS (SELECT COALESCE(SUM(amount),0)::numeric AS v FROM cash_deposits
                WHERE cash_session_id=v_cs.id AND status='active'
                  AND (LOWER(COALESCE(method,'efectivo'))='efectivo' OR method IS NULL)),
  stored_expected AS (SELECT COALESCE(v_cs.expected_amount, COALESCE(v_cs.opening_amount,0) + cs.v + dpc.v - exc.v - puc.v) AS v
                      FROM cash_sum cs, dep_cash dpc, exp_cash exc, pur_cash puc)
  SELECT jsonb_build_object(
    'total_sales', t.total_sales, 'order_count', t.order_count,
    'avg_ticket', CASE WHEN t.order_count>0 THEN (t.total_sales/t.order_count)::numeric ELSE 0 END,
    'cancelled_count', ca.n, 'cancelled_value', ca.v,
    'cash_total', cs.v, 'expenses_total', ex.v + pu.v, 'expenses_cash', exc.v + puc.v,
    ...
    'difference', COALESCE(v_cs.counted_amount,0) - se.v
  ) INTO v_summary
  FROM totals t, cancel_agg ca, cash_sum cs, exp_agg ex, exp_cash exc,
       pur_agg pu, pur_cash puc, dep_agg dp, dep_cash dpc, stored_expected se;
  ...
  SELECT COALESCE(jsonb_object_agg(method, jsonb_build_object('amount', amount, 'count', count)), '{}'::jsonb)
    INTO v_payments FROM (
      SELECT method, SUM(amount)::numeric AS amount,
             COUNT(DISTINCT sale_id) FILTER (WHERE is_primary)::int AS count
        FROM (SELECT * FROM split_parts UNION ALL SELECT * FROM single_parts) x GROUP BY method
    ) q;
  ...
  SELECT COALESCE(jsonb_object_agg(k, jsonb_build_object('amount', amt, 'count', cnt)), '{}'::jsonb)
    INTO v_services FROM (
      ...
      SELECT COALESCE(NULLIF(order_type,''), NULLIF(source,''),'mesa') AS k,
             SUM(total)::numeric AS amt, COUNT(*)::int AS cnt FROM scoped_sales
  ...
  SELECT COALESCE(jsonb_agg(m ORDER BY (m->>'created_at') DESC), '[]'::jsonb)
    INTO v_movements FROM (
      ...
      SELECT jsonb_build_object(
        ...
        'category','compra','description',COALESCE(p.supplier,p.notes,p.invoice_number,'Compra'),
        'method',COALESCE(p.payment_method,'efectivo'),'amount',p.total,'status','active') AS m
        FROM purchases p WHERE p.cash_session_id=v_cs.id
      UNION ALL
      SELECT jsonb_build_object(
        'id',d.id,'kind','deposit','created_at',d.created_at,'user_name',d.user_name,
        'category','depósito','description',d.description,
  ...
  RETURN jsonb_build_object(
    'session', to_jsonb(v_cs.*), 'branch_name', v_branch_name,
    'summary', v_summary, 'payments', v_payments, 'services', v_services,
    'products', v_products, 'movements', v_movements
  );
END; $$;
