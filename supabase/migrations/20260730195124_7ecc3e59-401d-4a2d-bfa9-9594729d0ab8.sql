...
BEGIN
  ...
  -- Catálogo real de la sede. Se deduplica por nombre (no se descartan las
  -- copias vinculadas), de lo contrario sedes que heredan productos de otra
  -- sede quedaban sin ellos y el bot decía que no existían.
  SELECT coalesce(jsonb_agg(x.item ORDER BY x.fav DESC, x.sort NULLS LAST, x.pname), '[]'::jsonb)
  INTO v_products
  FROM (
    SELECT DISTINCT ON (lower(btrim(p.name)))
      ...
      coalesce(p.is_favorite, false) AS fav,
    ...
    WHERE p.active = true
      ...
      AND (p.available_branch_ids IS NULL
           OR cardinality(p.available_branch_ids) = 0
           OR v_branch_id = ANY(p.available_branch_ids))
    ORDER BY lower(btrim(p.name)),
             (v_branch_id = ANY(coalesce(p.available_branch_ids, ARRAY[]::uuid[]))) DESC,
             (p.source_product_id IS NULL) DESC,
             p.created_at
  ) x;
  ...
  FROM (
    ...
    WHERE mg.branch_id = v_branch_id
    GROUP BY mg.name, mg.min_select, mg.max_select, mg.required
  ) g;
  ...
  RETURN jsonb_build_object(
    ...
    'welcome_messages', coalesce(to_jsonb(v_cfg.welcome_messages), '[]'::jsonb),
