...
CREATE INDEX IF NOT EXISTS idx_sales_scheduled_for
  ON public.sales(scheduled_for)
...
BEGIN
  ...
  _delivery_address := NULLIF(_payload->>'delivery_address','');
  _notes := NULLIF(_payload->>'notes','');
  _scheduled_for := NULLIF(_payload->>'scheduled_for','')::timestamptz;

  IF _scheduled_for IS NOT NULL THEN
    IF _scheduled_for <= now() THEN
      RAISE EXCEPTION 'La fecha programada debe ser futura';
    END IF;
    IF _scheduled_for > now() + interval '14 days' THEN
      RAISE EXCEPTION 'La fecha programada no puede superar 14 días';
    END IF;
  END IF;

  IF _branch_slug IS NOT NULL THEN
    SELECT id INTO _slug_branch_id FROM public.branches WHERE slug = _branch_slug LIMIT 1;
    IF _slug_branch_id IS NULL THEN
      RAISE EXCEPTION 'La sede del enlace no existe';
    END IF;
    IF _branch_id IS NOT NULL AND _branch_id <> _slug_branch_id THEN
      RAISE EXCEPTION 'La sede del enlace no coincide con el pedido';
    END IF;
    _branch_id := _slug_branch_id;
  END IF;
  ...
  IF _source = 'online_menu'
     AND _delivery_address IS NOT NULL
     AND COALESCE(_notes,'') NOT ILIKE '%RECOGER EN HELADER%' THEN
  ...
  IF _scheduled_for IS NOT NULL THEN
    _final_notes := CONCAT('[PROGRAMADO ',
      to_char(_scheduled_for AT TIME ZONE 'America/Bogota','YYYY-MM-DD HH24:MI'),
      ']',
  ...
  INSERT INTO public.sales (
    ...
    notes, payment_details, scheduled_for
  ) VALUES (
    NULL,
