...
DECLARE
  ...
  v_stale_instance boolean := false;
BEGIN
  ...
  -- Solo suavizamos parpadeos de la MISMA instancia durante segundos.
  -- Nunca ocultamos disconnected/error, y nunca ocultamos un QR de una instancia nueva.
  v_recent_same_instance_connected := v_same_instance
    AND v_cfg.connected_phone IS NOT NULL
    AND v_cfg.last_connected_at > (v_now - interval '30 seconds');

  -- Un proceso viejo no debe pisar a una instancia activa nueva, pero este bloqueo
  -- caduca rápido para permitir recuperación real si la instancia anterior murió.
  v_stale_instance := v_instance_id IS NOT NULL
    AND v_cfg.active_instance_id IS NOT NULL
    AND v_cfg.active_instance_id <> v_instance_id
    AND v_cfg.last_seen_at > (v_now - interval '45 seconds')
    AND v_cfg.connection_status = 'connected'
    AND _status IN ('connecting');

  IF v_stale_instance OR (_status = 'qr' AND v_recent_same_instance_connected) THEN
    ...
    RETURN jsonb_build_object(
      ...
      'ignored', true,
      'reason', CASE WHEN v_stale_instance THEN 'stale_instance_connecting_ignored' ELSE 'same_instance_qr_flicker_ignored' END,
...
DROP FUNCTION IF EXISTS public.whatsapp_bot_report_status(text, text, text, text);
...
BEGIN
  ...
  IF v_last_any IS NOT NULL AND v_last_any > v_now - v_cooldown THEN
    ...
    END IF;
    RETURN jsonb_build_object('reply', null, 'use_ai', v_ai_authorized, 'skipped', 'cooldown', 'skip_reason', 'cooldown');
  END IF;
  ...
  IF v_cfg.pickup_after_hours_enabled AND NOT v_online_open AND v_physical_open THEN
    ...
    RETURN jsonb_build_object('reply', null, 'skipped', 'pickup_after_hours_no_reply');
  END IF;
  ...
  IF v_cfg.after_hours_enabled AND NOT v_online_open AND NOT v_physical_open THEN
    ...
    RETURN jsonb_build_object('reply', null, 'skipped', 'after_hours_no_reply');
  END IF;
  ...
  IF v_reply = '' OR v_reply IS NULL THEN
    ...
    END IF;
    RETURN jsonb_build_object('reply', null, 'skipped', 'no_rule_no_ai', 'skip_reason', 'not_ai_authorized');
  END IF;
