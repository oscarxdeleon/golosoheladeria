...
-- Drop old signature (adding new _date param)
DROP FUNCTION IF EXISTS public.supervisor_dashboard_rpc(text, uuid, boolean);
...
BEGIN
  -- Validate session
  SELECT * INTO v_session FROM supervisor_sessions WHERE session_token = _session_token AND expires_at > now();
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Sesión de supervisor inválida o expirada';
  END IF;
  SELECT * INTO v_supervisor FROM supervisor_accounts WHERE id = v_session.supervisor_id AND active = true;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Acceso de supervisor no autorizado';
  END IF;

  -- Determine target date (default = today in Bogota)
  ...
  -- Determine branch: passed in, or first branch
  ...
  IF _log_switch THEN
    INSERT INTO supervisor_audit_log(supervisor_id, session_id, action, branch_id, metadata)
    VALUES (v_supervisor.id, v_session.id, 'switch_branch', v_branch_id, jsonb_build_object('date', v_target_date));
  END IF;
  ...
  -- Summary (mirrors dashboard.tsx logic)
  WITH day_sales AS (
    SELECT * FROM sales
  ...
  pay_single AS (
    ...
    FROM active_sales
    WHERE NOT (
      (payment_details->>'split')::boolean IS TRUE
  ...
  pay_all AS (
    SELECT method, SUM(amount)::numeric AS amount FROM (
      ...
      UNION ALL
      SELECT method, amount FROM pay_single
    ) x GROUP BY method
  ),
  totals AS (
    SELECT
  ...
  cancel_agg AS (
    SELECT COUNT(*)::int AS n, COALESCE(SUM(total),0)::numeric AS v FROM cancelled_sales
  ),
  exp_agg AS (
  ...
  pur_agg AS (
    SELECT COALESCE(SUM(total),0)::numeric AS v FROM purchases
    WHERE branch_id = v_branch_id AND created_at >= v_start AND created_at < v_end
  ),
  dep_agg AS (
    SELECT COALESCE(SUM(amount),0)::numeric AS v FROM cash_deposits
    WHERE branch_id = v_branch_id AND created_at >= v_start AND created_at < v_end
      AND status = 'active'
  ),
  tables_occ AS (
  ...
  tips_agg AS (
    SELECT COALESCE(SUM(tip_amount),0)::numeric AS v FROM active_sales
  )
  ...
  -- by_hour (local Bogota hour)
  SELECT COALESCE(jsonb_object_agg(hh, v ORDER BY hh), '{}'::jsonb) INTO v_by_hour FROM (
  ...
  -- by_service (order_type or source)
  ...
  -- by_payment (uses splits when present)
  ...
  -- top products grouped by product base name
  WITH day_sales AS (
  ...
  agg AS (
    SELECT
      COALESCE(
        NULLIF(p.name, ''),
        TRIM(SPLIT_PART(COALESCE(si.product_name,''), '+', 1))
      ) AS name,
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object('name', name, 'qty', qty, 'total', total)
    ...
    INTO v_top
    FROM (SELECT * FROM agg ORDER BY total DESC LIMIT 5) t;

  SELECT COALESCE(jsonb_agg(jsonb_build_object('name', name, 'qty', qty, 'total', total)
                            ORDER BY total ASC), '[]'::jsonb)
    INTO v_low
    FROM (
      WITH day_sales AS (
