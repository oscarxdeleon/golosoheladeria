ALTER TABLE public.whatsapp_bot_config
  ...
  ADD COLUMN IF NOT EXISTS active_instance_started_at timestamptz;

UPDATE public.whatsapp_bot_config
   SET last_connected_at = coalesce(last_connected_at, last_seen_at, now())
...
BEGIN
  ...
  v_recent_connected := v_cfg.last_connected_at IS NOT NULL
  ...
  IF _status = 'qr' AND v_recent_connected THEN
    UPDATE public.whatsapp_bot_config
       SET bot_version = coalesce(nullif(_version, ''), bot_version)
     WHERE branch_id = v_branch_id
