...
BEGIN
  ...
  IF v_branch_id IS NULL THEN
    RAISE EXCEPTION 'No hay sedes configuradas para mostrar en Supervisor';
  END IF;
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object('id', id, 'name', name, 'is_main', COALESCE(is_main, false))
  ...
  active_sales AS (
    SELECT * FROM day_sales WHERE status <> 'cancelled'
  ),
  cancelled_sales AS (
    SELECT * FROM day_sales WHERE status = 'cancelled'
  ),
  pay_split AS (
    ...
    LATERAL jsonb_array_elements(
      CASE
        WHEN (s.payment_details->>'split')::boolean IS TRUE
  ...
  totals AS (
    SELECT COALESCE(SUM(total), 0)::numeric AS total_sales, COUNT(*)::int AS order_count
    FROM active_sales
  ),
  cash_sum AS (
    SELECT COALESCE(SUM(amount), 0)::numeric AS v FROM pay_all WHERE method = 'efectivo'
  ),
  digital_sum AS (
    SELECT COALESCE(SUM(amount), 0)::numeric AS v FROM pay_all WHERE method <> 'efectivo'
  ),
  ...
  SELECT COALESCE(jsonb_object_agg(hh, v), '{}'::jsonb)
  INTO v_by_hour
  FROM (
  ...
  FROM (
    ...
    sp AS (
      SELECT
        LOWER(TRIM(COALESCE(x->>'method', 'otro'))) AS method,
  ...
  agg AS (
    SELECT
      COALESCE(NULLIF(p.name, ''), TRIM(SPLIT_PART(COALESCE(si.product_name, ''), '+', 1))) AS name,
  ...
  WHERE cs.branch_id = v_branch_id
    AND (
      (cs.closed_at IS NOT NULL AND cs.closed_at >= v_start AND cs.closed_at < v_end)
