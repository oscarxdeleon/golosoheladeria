...
-- Distinguir "domicilio cerrado pero heladería abierta para recoger/consumir en local"
ALTER TABLE public.whatsapp_bot_config
  ADD COLUMN IF NOT EXISTS pickup_after_hours_enabled boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS pickup_after_hours_messages text[] NOT NULL DEFAULT ARRAY[
    '¡Hola! 🍨 En este momento el *servicio a domicilio* ya está cerrado, pero seguimos atendiendo en la heladería para *recoger o consumir en el local*. También puedes dejar tu pedido programado para mañana 👉 {menu_link}',
    '¡Hola! 👋 Por ahora no estamos haciendo domicilios, pero la heladería sigue abierta si quieres pasar a *recoger tu pedido* o *disfrutarlo aquí*. Si prefieres, programa tu domicilio para más tarde 👉 {menu_link}'
  ]::text[];

CREATE OR REPLACE FUNCTION public.whatsapp_bot_is_physical_open(_branch_id uuid)
RETURNS boolean
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.whatsapp_bot_is_physical_open(uuid) TO anon, authenticated;
...
BEGIN
  ...
  -- Caso 1: domicilio cerrado PERO heladería abierta (recoger / consumir en local)
  IF NOT v_online_open AND v_physical_open AND v_cfg.pickup_after_hours_enabled THEN
    IF v_should_greet AND array_length(v_cfg.pickup_after_hours_messages,1) > 0 THEN
      v_idx := 1 + (abs(hashtext(_from || current_date::text)) % array_length(v_cfg.pickup_after_hours_messages,1));
  ...
  -- Caso 2: domicilio cerrado y heladería también cerrada
