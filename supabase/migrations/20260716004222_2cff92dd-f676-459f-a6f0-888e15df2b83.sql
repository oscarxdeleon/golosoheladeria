...
BEGIN
  ...
  IF NOT (_is_admin OR _is_cajero) THEN
  ...
  IF _was_paid AND NOT _is_admin THEN
    RAISE EXCEPTION 'Este pedido ya fue pagado. Solo el administrador puede anularlo (requiere reversión).';
  END IF;
  ...
  -- Reponer inventario: por cada item con producto que rastrea stock,
  -- sumar qty al stock y registrar movimiento de 'entrada' con reversión.
  FOR _item IN
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'cancelled_by_name', COALESCE(_uname, 'Cajero'),
      'cancellation_reason', _reason_trim, 'table_released', _table_released,
      'inventory_reverted_items', _inv_reverted
    ),
    jsonb_build_object(
      'source', 'cancel_sale_rpc', 'reason', _reason_trim,
