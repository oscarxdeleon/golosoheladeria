...
BEGIN
  ...
  -- REGLA DE WHITELIST DEFINITIVA:
  -- Si el administrador configuró números de sandbox, SOLO esos números
  -- reciben respuesta, sin importar si ai_ordering_enabled está activo.
  -- El modo "abierto a todos" requiere lista de sandbox vacía.
...
-- Ajustamos también whatsapp_bot_handle_incoming para que:
-- 1) Si hay sandbox, los no autorizados NO reciben ninguna respuesta.
-- 2) Los saludos cortos devuelven la bienvenida directamente (sin use_ai)
--    para evitar el mensaje duplicado.
...
BEGIN
  ...
  -- WHITELIST GLOBAL: si hay números de prueba configurados, ningún otro
  -- número recibe respuesta (ni fija ni IA). Esto vale también para el bot
  -- operacional, no solo para la IA.
  IF NOT v_sandbox_empty THEN
  ...
  IF v_last_any IS NOT NULL AND v_last_any > v_now - v_cooldown THEN
    IF v_is_greeting THEN
      -- En cooldown NO repetimos el saludo. Silenciamos para evitar duplicados.
  ...
  -- Saludo: SIEMPRE respondemos con la bienvenida directamente (fija, sin IA)
  -- para evitar que después el flujo de ai_reply envíe un segundo saludo.
  IF v_is_greeting THEN
