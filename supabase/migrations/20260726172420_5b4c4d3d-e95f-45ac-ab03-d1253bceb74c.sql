-- Normalizar sesiones activas de WhatsApp y evitar duplicados por número
UPDATE public.whatsapp_ai_carts c
SET status = 'expired', updated_at = now(), expires_at = coalesce(c.expires_at, now())
WHERE c.status = 'building'
  AND c.id NOT IN (
    SELECT DISTINCT ON (branch_id, public.whatsapp_bot_contact_key(phone)) id
    FROM public.whatsapp_ai_carts
    WHERE status = 'building'
      AND coalesce(expires_at, updated_at + interval '45 minutes') > now()
    ORDER BY branch_id, public.whatsapp_bot_contact_key(phone), updated_at DESC, id DESC
  );

DROP INDEX IF EXISTS public.idx_wa_ai_carts_active_phone;
CREATE INDEX IF NOT EXISTS idx_wa_ai_carts_building_phone
  ON public.whatsapp_ai_carts (branch_id, public.whatsapp_bot_contact_key(phone), updated_at DESC)
  WHERE status = 'building';

CREATE UNIQUE INDEX IF NOT EXISTS idx_wa_ai_carts_one_building_per_contact
  ON public.whatsapp_ai_carts (branch_id, public.whatsapp_bot_contact_key(phone))
  WHERE status = 'building';

CREATE INDEX IF NOT EXISTS idx_wa_ai_messages_contact_recent
  ON public.whatsapp_ai_messages (branch_id, public.whatsapp_bot_contact_key(phone), created_at DESC);
...
BEGIN
  ...
  -- GATE MAESTRO: durante pedido/conversación activa, nunca ejecutar plantillas fijas.
  SELECT EXISTS(
  ...
  IF v_cfg.pickup_after_hours_enabled AND NOT v_online_open AND v_physical_open THEN
    ...
    IF v_len > 0 THEN
      ...
      UPDATE public.whatsapp_bot_greeted SET last_greeted_at = v_now, last_after_hours_at = v_now, last_menu_at = v_now, last_msg_idx = v_idx, greeted_date = current_date WHERE branch_id = v_cfg.branch_id AND public.whatsapp_bot_contact_key(phone) = v_contact_key;
