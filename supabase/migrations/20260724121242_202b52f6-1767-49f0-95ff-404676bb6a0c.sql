ALTER TABLE public.whatsapp_bot_config
  ADD COLUMN IF NOT EXISTS ai_daily_limit_per_phone integer;

CREATE TABLE IF NOT EXISTS public.whatsapp_ai_diagnostics (
...
GRANT SELECT ON public.whatsapp_ai_diagnostics TO authenticated;
GRANT ALL ON public.whatsapp_ai_diagnostics TO service_role;

ALTER TABLE public.whatsapp_ai_diagnostics ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Admins y supervisores ven diagnostico IA WhatsApp" ON public.whatsapp_ai_diagnostics;
CREATE POLICY "Admins y supervisores ven diagnostico IA WhatsApp"
  ON public.whatsapp_ai_diagnostics FOR SELECT
  TO authenticated
  USING (
...
CREATE INDEX IF NOT EXISTS idx_whatsapp_ai_diag_branch_created
  ON public.whatsapp_ai_diagnostics (branch_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_whatsapp_ai_diag_conversation
  ON public.whatsapp_ai_diagnostics (conversation_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_whatsapp_ai_diag_errors
  ON public.whatsapp_ai_diagnostics (branch_id, created_at DESC)
  WHERE ok = false;

CREATE OR REPLACE FUNCTION public.whatsapp_bot_ai_log_event(
  _token text,
...
BEGIN
  ...
  INSERT INTO public.whatsapp_ai_diagnostics(
    branch_id, conversation_id, phone, stage, ok, duration_ms, error, metadata
  ) VALUES (
    v_branch_id,
    left(coalesce(nullif(_conversation_id, ''), gen_random_uuid()::text), 120),
    nullif(v_phone_clean, ''),
    left(coalesce(nullif(_stage, ''), 'unknown'), 120),
    coalesce(_ok, true),
    _duration_ms,
    left(coalesce(_error, ''), 1000),
...
REVOKE ALL ON FUNCTION public.whatsapp_bot_ai_log_event(text, text, text, text, boolean, integer, text, jsonb) FROM public;
GRANT EXECUTE ON FUNCTION public.whatsapp_bot_ai_log_event(text, text, text, text, boolean, integer, text, jsonb) TO anon, authenticated, service_role;
...
BEGIN
  ...
  -- Sin límite por defecto: reply_count queda como telemetría. Si en el futuro se configura
  -- ai_daily_limit_per_phone con un valor positivo, se aplica como protección explícita.
