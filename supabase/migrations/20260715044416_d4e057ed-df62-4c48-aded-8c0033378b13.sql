-- Fix: expenses table uses payment_method (not method) and has no status column.
-- Both supervisor RPCs referenced `expenses.method` and `expenses.status`, causing
-- "column method does not exist" when the Supervisor dashboard/detail loaded.

DO $$ BEGIN PERFORM 1; END $$;

-- Re-create both RPCs with corrected column references.
-- (cash_deposits still uses `method` and `status` — those references are preserved.)

DROP FUNCTION IF EXISTS public.supervisor_dashboard_rpc(text, uuid, boolean, date);
DROP FUNCTION IF EXISTS public.supervisor_session_detail_rpc(text, uuid);

-- The full function bodies are re-created below with:
--   expenses.method        -> expenses.payment_method
--   COALESCE(e.status,'x') -> 'active'
--   d.method / d.status    -> unchanged (cash_deposits has these columns)
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'expenses', ex.v, 'expenses_cash', exc.v, 'purchases', 0,
    'deposits', dp.v, 'deposits_cash', dpc.v,
    'entries', dp.v, 'exits', ex.v, 'refunds', 0, 'tips', tp.v,
  ...
  SELECT COALESCE(jsonb_object_agg(k,v),'{}'::jsonb) INTO v_by_service FROM (
    SELECT COALESCE(NULLIF(order_type,''), NULLIF(source,''),'mesa') AS k, SUM(total)::numeric AS v
      FROM sales WHERE branch_id=v_branch_id AND created_at>=v_start AND created_at<v_end AND status<>'cancelled'
     GROUP BY 1) q;

  SELECT COALESCE(jsonb_object_agg(method, amount),'{}'::jsonb) INTO v_by_payment FROM (
    WITH day_sales AS (SELECT * FROM sales WHERE branch_id=v_branch_id AND created_at>=v_start AND created_at<v_end AND status<>'cancelled'),
    sp AS (
    ...
    single AS (
      SELECT LOWER(TRIM(COALESCE(payment_method,'otro'))) AS method, total AS amount FROM day_sales
       WHERE NOT ((payment_details->>'split')::boolean IS TRUE AND jsonb_typeof(payment_details->'splits')='array')
          OR payment_details IS NULL)
    SELECT method, SUM(amount)::numeric AS amount FROM (SELECT * FROM sp UNION ALL SELECT * FROM single) u GROUP BY method
  ) q;

  WITH day_sales AS (SELECT id FROM sales WHERE branch_id=v_branch_id AND created_at>=v_start AND created_at<v_end AND status<>'cancelled'),
  agg AS (
...
BEGIN
  ...
  WITH s AS (SELECT * FROM sales WHERE branch_id=v_cs.branch_id AND created_at>=v_start AND created_at<v_end),
  ...
  pay_split AS (
    SELECT LOWER(TRIM(COALESCE(sp->>'method','otro'))) AS method,
      ...
      FROM active_sales x, LATERAL jsonb_array_elements(
  ...
  pay_single AS (
    SELECT LOWER(TRIM(COALESCE(payment_method,'otro'))) AS method, total AS amount, 1 AS cnt
     ...
     WHERE NOT ((payment_details->>'split')::boolean IS TRUE AND jsonb_typeof(payment_details->'splits')='array')
        OR payment_details IS NULL),
  pay_all AS (SELECT method, SUM(amount)::numeric AS amount, SUM(cnt)::int AS count FROM (
  ...
  WITH s AS (SELECT * FROM sales WHERE branch_id=v_cs.branch_id AND created_at>=v_start AND created_at<v_end AND status<>'cancelled'),
  pay_split AS (
    SELECT LOWER(TRIM(COALESCE(sp->>'method','otro'))) AS method,
           COALESCE((sp->>'amount')::numeric,0) AS amount, 1 AS cnt
      FROM s x, LATERAL jsonb_array_elements(
             CASE WHEN (x.payment_details->>'split')::boolean IS TRUE
                       AND jsonb_typeof(x.payment_details->'splits')='array'
                  THEN x.payment_details->'splits' ELSE '[]'::jsonb END) sp),
  pay_single AS (
    SELECT LOWER(TRIM(COALESCE(payment_method,'otro'))) AS method, total AS amount, 1 AS cnt FROM s
     WHERE NOT ((payment_details->>'split')::boolean IS TRUE AND jsonb_typeof(payment_details->'splits')='array')
        OR payment_details IS NULL)
  SELECT COALESCE(jsonb_object_agg(method, jsonb_build_object('amount', amount, 'count', count)), '{}'::jsonb)
    INTO v_payments FROM (SELECT method, SUM(amount)::numeric AS amount, SUM(cnt)::int AS count
  ...
  SELECT COALESCE(jsonb_object_agg(k, jsonb_build_object('amount', amt, 'count', cnt)), '{}'::jsonb)
    INTO v_services FROM (
      SELECT COALESCE(NULLIF(order_type,''), NULLIF(source,''),'mesa') AS k,
             SUM(total)::numeric AS amt, COUNT(*)::int AS cnt FROM sales
