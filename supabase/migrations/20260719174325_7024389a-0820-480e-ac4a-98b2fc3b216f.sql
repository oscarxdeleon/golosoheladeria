...
-- 1) Limpiar historial: eliminar entradas del bot que en realidad son
--    actualizaciones de Estado/Historias de WhatsApp o remitentes no numéricos.
DELETE FROM public.whatsapp_bot_messages
WHERE from_number = 'status'
   OR from_number ILIKE '%@broadcast'
   OR from_number ILIKE '%@g.us'
   OR from_number ILIKE '%@newsletter'
   OR from_number !~ '^[0-9]{6,}$';

-- También limpiar registros "greeted" asociados a esos falsos remitentes.
DELETE FROM public.whatsapp_bot_greeted
WHERE phone = 'status'
   OR phone !~ '^[0-9]{6,}$';

-- 2) Red de seguridad server-side: el RPC filtra estos JIDs para bots
--    antiguos que aún no tienen el fix del cliente.
...
BEGIN
  ...
  -- Ignorar silenciosamente estados/historias, grupos, broadcasts, newsletters
  -- y cualquier remitente que no sea un número de teléfono real. Esto evita
  -- registrar en el historial los mensajes de "status@broadcast".
