...
BEGIN
  ...
  WITH scoped_sales AS (
    ...
    WHERE s.branch_id = _active_branch
      ...
      AND (
        (_active_cash_row.id IS NOT NULL AND s.cash_session_id = _active_cash_row.id)
        OR (
          (_active_cash_row.id IS NULL OR s.cash_session_id IS NULL)
    ...
    WHERE e.branch_id = _active_branch
      AND (
        (_active_cash_row.id IS NOT NULL AND e.cash_session_id = _active_cash_row.id)
        OR (
          (_active_cash_row.id IS NULL OR e.cash_session_id IS NULL)
          AND e.created_at >= _start_at
          AND e.created_at <= _end_at
        )
      )
  ), scoped_deposits AS (
    SELECT cd.*
    ...
    WHERE cd.branch_id = _active_branch
      ...
      AND (
        (_active_cash_row.id IS NOT NULL AND cd.cash_session_id = _active_cash_row.id)
        OR (
          (_active_cash_row.id IS NULL OR cd.cash_session_id IS NULL)
          AND cd.created_at >= _start_at
          AND cd.created_at <= _end_at
        )
      )
    ...
    SELECT
      COALESCE(sum(amount) FILTER (WHERE lower(COALESCE(category,'')) IN ('ingreso','entrada','propina')), 0) AS entries,
      COALESCE(sum(amount) FILTER (WHERE lower(COALESCE(category,'')) IN ('retiro','salida')), 0) AS exits,
      COALESCE(sum(amount) FILTER (WHERE lower(COALESCE(category,'')) IN ('devolucion','devolución','reembolso')), 0) AS refunds,
      COALESCE(sum(amount) FILTER (WHERE lower(COALESCE(category,'')) NOT IN ('ingreso','entrada','propina','retiro','salida','devolucion','devolución','reembolso')), 0) AS expenses
  ...
  WITH scoped_sales AS (
    ...
    WHERE s.branch_id = _active_branch
      ...
      AND (
        (_active_cash_row.id IS NOT NULL AND s.cash_session_id = _active_cash_row.id)
        OR ((_active_cash_row.id IS NULL OR s.cash_session_id IS NULL) AND s.created_at >= _start_at AND s.created_at <= _end_at)
      )
  )
